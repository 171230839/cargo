    ...
    for unit in units.iter() {
        ...
        for dep in cx.dep_targets(unit)?.iter() {
            ...
            cx.compilation
                ...
                .or_insert(HashSet::new())
                .extend(v.iter().map(
                    |&(ref f, _, _)| (dep.target.clone(), f.clone()),
                ));
        }
...
fn rustc<'a, 'cfg>(
    ...
    return Ok(Work::new(move |state| {
        ...
        if json_messages {
            exec.exec_json(
                ...
                &mut |line| if !line.is_empty() {
                    Err(internal(
                        &format!("compiler stdout is not empty: `{}`", line),
                    ))
                } else {
                    Ok(())
                },
        ...
        if do_rename && real_name != crate_name {
            ...
            let src = dst.with_file_name(dst.file_name().unwrap().to_str().unwrap().replace(
                &real_name,
                &crate_name,
            ));
...
fn rustdoc<'a, 'cfg>(cx: &mut Context<'a, 'cfg>, unit: &Unit<'a>) -> CargoResult<Work> {
    ...
    Ok(Work::new(move |state| {
        ...
        rustdoc.exec().chain_err(
            || format!("Could not document `{}`.", name),
        )
    }))
}
...
fn build_base_args(cx: &mut Context, cmd: &mut ProcessBuilder, unit: &Unit, crate_types: &[&str]) {
    ...
    let prefer_dynamic = (unit.target.for_host() && !unit.target.is_custom_build()) ||
        (crate_types.contains(&"dylib") && cx.ws.members().find(|&p| p != unit.pkg).is_some());
    if prefer_dynamic {
    ...
    // for (key, value) in unit.pkg.summary().cfgs(){
    //     println!("key : {}, value: {}", key, value);
    //     cmd.arg("--cfg").arg(&format!("{}=\"{}\"", key, value ));
    // }
    for (key, value) in cx.ws.cfgs(){
            // println!("key : {}, value: {}", key, value);
        cmd.arg("--cfg").arg(&format!("{}=\"{}\"", key, value )); 
    }
